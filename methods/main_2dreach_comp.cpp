...
int main(int argc, char **argv) {
    ...
    vector<const RTree *> rTreeForNode(numNodes, nullptr);
    for (ID nid = 0; nid < numNodes; ++nid) {
        if (auto it = subgraph.compToSubgraphComp[G.nodes[nid].cid]; it.has_value()) {
            const auto &treePtr = componentRTrees[it.value()];
            if (treePtr) {
                rTreeForNode[nid] = treePtr.get();
            }
        }
    }
    ...
    size_t rTreeForNodeBytes = rTreeForNode.size() * sizeof(const RTree *);
    ...
    while (inp >> nid >> xlow >> ylow >> xhigh >> yhigh) {
        ...
        if (const RTree *treePtr = rTreeForNode[nid]) {
    ...
    cout << "Method                   : 2DReach-Comp" << endl << endl;
